...
sl::DLSSDPreset mapDlssdPreset(uint32_t preset) {
    if (preset == static_cast<uint32_t>(sl::DLSSDPreset::eDefault)
            ...
            || preset == static_cast<uint32_t>(sl::DLSSDPreset::ePresetE)) {
...
SLBRIDGE_EXPORT int32_t slbridge_tag_resources(uint64_t frame_token, uint32_t viewport,
    ...
    for (uint32_t i = 0; i < resource_count; i++) {
        ...
        const uint32_t extentWidth = descriptor.extent_width != 0
                ? descriptor.extent_width : descriptor.width;
...
SLBRIDGE_EXPORT int32_t slbridge_get_dlssd_optimal_settings(uint32_t mode,
        ...
        uint32_t* out_render_width_min, uint32_t* out_render_height_min,
        uint32_t* out_render_width_max, uint32_t* out_render_height_max) {
    if (!g_slGetFeatureFunction || !out_render_width || !out_render_height || !out_sharpness
            || !out_render_width_min || !out_render_height_min
            || !out_render_width_max || !out_render_height_max
    ...
    if (result == static_cast<int32_t>(sl::Result::eOk)) {
        ...
        *out_render_width_min = settings.renderWidthMin;
        *out_render_height_min = settings.renderHeightMin;
        *out_render_width_max = settings.renderWidthMax;
