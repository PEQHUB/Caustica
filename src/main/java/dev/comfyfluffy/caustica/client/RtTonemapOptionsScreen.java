...
import dev.comfyfluffy.caustica.client.ui.NumericValueScreen;
...
public final class RtTonemapOptionsScreen extends Screen {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        if (event.button() == 1 || (event.button() == 0 && event.hasControlDown())) {
            for (Map.Entry<AbstractWidget, RtVideoOptions.TonemapControl> entry : resetControls.entrySet()) {
                AbstractWidget widget = entry.getKey();
                RtVideoOptions.NumericEditor editor = entry.getValue().numericEditor();
                if (editor != null && widget.visible && widget.active && widget.isMouseOver(event.x(), event.y())) {
                    minecraft.setScreenAndShow(new NumericValueScreen(this, widget.getMessage(),
                            editor.getter().getAsDouble(), value -> {
                                editor.set(value);
                                saveSettings();
                                rebuildWidgets();
                            }));
    ...
    private static List<Section> visibleSections() {
        ...
        if (hdrEnabled) {
            sections.add(section("hdrOutput", controls(output[2], output[3], output[4], output[5])));
        }
