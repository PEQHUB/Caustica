...
public final class RtSkyViewPipeline {
    ...
    public record Parameters(float rayleigh, float aerosolScatter, float aerosolAbsorption, float ozone,
                             float aerosolHeightKm, float aerosolAnisotropy,
                             float brightnessScale, float saturation,
    ...
    public static RtSkyViewPipeline create(RtContext ctx) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            VkPushConstantRange.Buffer range = VkPushConstantRange.calloc(1, stack)
                    .stageFlags(VK10.VK_SHADER_STAGE_COMPUTE_BIT).offset(0).size(88);
    ...
    public void dispatchTransmittance(VkCommandBuffer cmd, int width, int height, Parameters parameters) {
        dispatch(cmd, width, height, 0, 0, 0, 0, 0, 0, 0, 0, true, 0, parameters);
    }
    ...
    public void dispatchSky(VkCommandBuffer cmd, int width, int height,
                          ...
                          boolean enabled, Parameters parameters) {
        dispatch(cmd, width, height, sunX, sunY, sunZ, sunSource,
                moonX, moonY, moonZ, moonSource, enabled, 1, parameters);
    }

    private void dispatch(VkCommandBuffer cmd, int width, int height,
                           float sunX, float sunY, float sunZ, float sunSource,
                           float moonX, float moonY, float moonZ, float moonSource,
                           boolean enabled, int pass, Parameters parameters) {
        try (MemoryStack stack = MemoryStack.stackPush();
            ...
            ByteBuffer push = stack.malloc(88);
            ...
            push.putFloat(32, parameters.rayleigh()); push.putFloat(36, parameters.aerosolScatter());
            push.putFloat(40, parameters.aerosolAbsorption()); push.putFloat(44, parameters.ozone());
            push.putFloat(48, parameters.aerosolHeightKm()); push.putFloat(52, parameters.aerosolAnisotropy());
            push.putFloat(56, parameters.brightnessScale()); push.putFloat(60, parameters.saturation());
            push.putFloat(64, parameters.tintR()); push.putFloat(68, parameters.tintG());
            push.putFloat(72, parameters.tintB()); push.putFloat(76, 0.0f);
            push.putInt(80, enabled ? 1 : 0);
            push.putInt(84, pass);
