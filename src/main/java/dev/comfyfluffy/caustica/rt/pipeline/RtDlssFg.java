...
public final class RtDlssFg {
    ...
    private int currentInputSlot = DlssgInputPool.NO_SLOT;
    ...
    private long presentedCadenceSamples;
    private long expectedPresentedFrameTotal;
    private long actualPresentedFrameTotal;
    ...
    public int inputSlotCount() {
        return inputPool.capacity();
    }

    public int currentInputSlot() {
        return currentInputSlot;
    }

    public long inputPoolGeneration() {
        return inputPool.generation();
    }

    public int acquiredApplicationImage() {
        return acquiredApplicationImage;
    }

    public String requestedQueueMode() {
        return requestedQueuePolicy().id;
    }

    public String effectiveQueueMode() {
        return useNoClientQueues() ? "parallel" : "synchronized";
    }
    ...
    public String queuePolicyReason() {
        ...
        if (queuePolicyTransitionFrame) {
            return "One-frame suspension while changing queue ownership";
        }
        return activeQueuePolicy == QueuePolicy.PARALLEL
                ? "Client queues remain unblocked; tagged inputs use Streamline timeline retirement"
                : "Streamline blocks the presenting client queue until frame-generation work completes";
    }

    public boolean generationActiveForPresent() {
        return optionsEnabled && frameInputsSubmitted;
    }

    public boolean parallelQueueActive() {
        return useNoClientQueues();
    }
    ...
    public String inputSlotRetirements() {
        return "free=" + inputPool.freeCount()
                + ",capturing=" + inputPool.capturingCount()
                + ",ready=" + inputPool.readyCount()
                + ",pending=" + inputPool.pendingCount()
                + ",quarantined=" + inputPool.quarantinedCount();
    }
    ...
    public int outputTargetFps() {
        return CausticaConfig.Rt.Fg.OUTPUT_TARGET_FPS.value();
    }

    public int totalFrameMultiplier() {
        return effectiveMultiFrameCount() + 1;
    }

    public double renderedTargetFps() {
        return outputTargetFps() / (double) totalFrameMultiplier();
    }

    public double generatedFrameDropPercent() {
        if (expectedPresentedFrameTotal <= 0L) {
            return 0.0;
        }
        long missing = Math.max(0L, expectedPresentedFrameTotal - actualPresentedFrameTotal);
        return 100.0 * missing / expectedPresentedFrameTotal;
    }

    public long presentedCadenceSamples() {
        return presentedCadenceSamples;
    }

    /** True only while Reflex intentionally replaces Minecraft's ordinary end-of-frame limiter. */
    public boolean parallelPacingActive() {
        return requested() && isAvailable() && useNoClientQueues() && !shouldSuspendForMenu()
    ...
    public void beginSimulationFrame() {
        ...
        frameToken = 0L;
        currentInputSlot = DlssgInputPool.NO_SLOT;
        currentFrameIndex = -1;
    ...
    public void onSwapchainConfigured(int width, int height, int format, int imageCount,
        ...
        presentedCadenceSamples = 0L;
        expectedPresentedFrameTotal = 0L;
        actualPresentedFrameTotal = 0L;
    ...
    public void suspendForSwapchainChange() {
        drainInputPoolAtLifecycleBoundary(InputPoolBoundary.SWAPCHAIN_CHANGE);
        setOff(false);
        frameInputsSubmitted = false;
        frameToken = 0L;
    }
    ...
    public int beginFrameInputCapture() {
        if (dlssgFailed) {
            return DlssgInputPool.NO_SLOT;
        }
        if (!prepareQueuePolicyForFrame()) {
            currentInputSlot = DlssgInputPool.NO_SLOT;
            return DlssgInputPool.NO_SLOT;
        }
        retireInputPoolNonblocking();
        if (frameToken == 0L || frameLifecycle.outOfBand()) {
            currentInputSlot = DlssgInputPool.NO_SLOT;
            return DlssgInputPool.NO_SLOT;
        }
        if (currentInputSlot != DlssgInputPool.NO_SLOT
                && inputPool.isCapturing(currentInputSlot, frameToken, inputPool.generation())) {
            return currentInputSlot;
        }
        currentInputSlot = inputPool.tryAcquire(frameToken);
        if (currentInputSlot == DlssgInputPool.NO_SLOT) {
            optionsEnabled = false;
            forceResetNextSubmission = true;
    ...
    public void beforeInputResourcesDestroyed() {
        drainInputPoolAtLifecycleBoundary(InputPoolBoundary.INPUT_RESOURCE_DESTRUCTION);
    }
    ...
    public void afterPresent(int presentResult) {
        ...
        if (RtFrameStats.enabled()) {
            RtFrameStats.FRAME.count("fgFramesPresented",
        ...
        finalizeInputSlot(effectiveError);
        currentInputSlot = DlssgInputPool.NO_SLOT;
        frameToken = 0L;
        currentFrameIndex = -1;
        frameInputsSubmitted = false;
    ...
    public boolean submitFrame(int expectedSlot, long commandBuffer, int colorWidth, int colorHeight, int colorFormat,
        ...
        if (expectedSlot != currentInputSlot
                || !inputPool.isCapturing(expectedSlot, frameToken, inputPool.generation())) {
            submissionStatus = "Input lease changed before Streamline submission";
            return false;
        }
        ...
        try {
            ...
            if (currentInputSlot != DlssgInputPool.NO_SLOT) {
                if (!inputPool.markReady(currentInputSlot, frameToken)) {
                    inputPool.quarantine(currentInputSlot, frameToken);
                    submissionStatus = "Input lease changed before Streamline submission";
                    return false;
                }
            }
    ...
    private void applyReflexOptions(StreamlineLibrary library, StreamlineScratch scratch) {
        ...
        // internally reduce the host render cadence for the active multiplier. Dividing here would apply
        // the multiplier twice and produce the observed 1/x output-rate regression. Synchronized mode
        // retains its presenting-queue backpressure and needs no extra cap.
        int limit = parallelPacingActive()
                ? reflexLimitForDisplayedTarget(outputTargetFps())
                : requested() ? 0 : Math.max(0, CausticaConfig.Rt.Reflex.MINIMUM_INTERVAL_US.value());
    ...
    private void refreshState() {
        ...
        }
        {
            ...
            if (frameInputsSubmitted && optionsEnabled) {
                presentedCadenceSamples++;
                expectedPresentedFrameTotal += totalFrameMultiplier();
                actualPresentedFrameTotal += Math.max(1, framesActuallyPresented);
            }
            if (generatedNow > 0) {
            ...
            if (frameInputsSubmitted && optionsEnabled && useNoClientQueues()
                    && currentInputSlot != DlssgInputPool.NO_SLOT) {
                if (inputsProcessingFence != 0L && inputsProcessingFenceValue != 0L) {
                    inputPool.markPending(currentInputSlot, frameToken, inputsProcessingFence,
    ...
    private boolean prepareQueuePolicyForFrame() {
        if (dlssgFailed || queueFallback || queuePolicyTransitionFrame) {
            return false;
        }
        QueuePolicy requested = requestedQueuePolicy();
        if (requested == activeQueuePolicy) {
            return true;
        }
        ...
        retireInputPoolNonblocking();
        if (currentInputSlot != DlssgInputPool.NO_SLOT
                || inputPool.capturingCount() != 0
                || inputPool.readyCount() != 0
                || inputPool.pendingCount() != 0
                || inputPool.quarantinedCount() != 0) {
            submissionStatus = "Queue ownership change deferred until input ownership retires";
            optionsEnabled = false;
            forceResetNextSubmission = true;
            return false;
        }
        ...
        forceResetNextSubmission = true;
        submissionStatus = "Queue ownership changed; generation suspended for one present";
        return false;
    }

    private void drainInputPoolAtLifecycleBoundary(InputPoolBoundary boundary) {
        retireInputPoolNonblocking();
        if (inputPool.capturingCount() != 0 || inputPool.readyCount() != 0
                || inputPool.pendingCount() != 0 || inputPool.quarantinedCount() != 0) {
            if (StreamlineRuntime.initialized() && RenderSystem.getDevice() != null
                ...
                int result = StreamlineRuntime.vkDeviceWaitIdle(
                        device.vkDevice(), "DLSSG input pool lifecycle drain: " + boundary.id, false);
                if (result != RESULT_OK) {
                    CausticaMod.LOGGER.error("Could not quiesce Streamline inputs for {}: {}", boundary.id,
                            StreamlineRuntime.lastError());
                }
            }
            inputPool.releaseAllAfterDeviceIdle();
        }
        currentInputSlot = DlssgInputPool.NO_SLOT;
    }

    private void retireInputPoolNonblocking() {
    ...
    private void finalizeInputSlot(int presentResult) {
        if (currentInputSlot == DlssgInputPool.NO_SLOT || frameToken == 0L) return;
        if (!frameInputsSubmitted) {
            inputPool.releaseWithoutVendorOwnership(currentInputSlot, frameToken);
        } else if (presentResult != RESULT_OK && !isRecoverablePresentResult(presentResult)) {
            inputPool.quarantine(currentInputSlot, frameToken);
        } else if (useNoClientQueues()) {
            // A valid parallel submission remains RETIREMENT_PENDING; a missing completion is quarantined.
            if (inputPool.state(currentInputSlot) == DlssgInputPoolState.READY_FOR_PRESENT) {
                inputPool.quarantine(currentInputSlot, frameToken);
            }
        } else {
            inputPool.releaseSubmittedSynchronously(currentInputSlot, frameToken);
        }
    }

    private enum InputPoolBoundary {
        SWAPCHAIN_CHANGE("swapchain change"),
        INPUT_RESOURCE_DESTRUCTION("input resource destruction"),
        SHUTDOWN("shutdown");

        private final String id;

        InputPoolBoundary(String id) { this.id = id; }
    }

    private boolean useNoClientQueues() {
        return activeQueuePolicy == QueuePolicy.PARALLEL && !queueFallback;
    }

    private static QueuePolicy requestedQueuePolicy() {
        String value = System.getProperty("caustica.streamline.queueParallelism",
                        CausticaConfig.Rt.Fg.QUEUE_PARALLELISM.get())
                .trim().toLowerCase(java.util.Locale.ROOT);
        return QueuePolicy.from(value);
    }

    static String canonicalQueuePolicy(String value) {
        return QueuePolicy.from(value == null ? "" : value).id;
    }
    ...
    /** Passes the final displayed target to Reflex; DLSS-G owns multiplier-aware host pacing. */
    static int reflexLimitForDisplayedTarget(int outputTargetFps) {
        int target = Math.max(1, outputTargetFps);
        return Math.max(1, (int) Math.round(1_000_000.0 / target));
    }
    ...
    public void destroy() {
        drainInputPoolAtLifecycleBoundary(InputPoolBoundary.SHUTDOWN);
